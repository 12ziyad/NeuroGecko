...
| **Gaze stabilisation (optokinetic reflex)** | **Binocular hOKR gain: 0.9 at 20°/s, 0.8 at 30°/s, 0.7–0.8 at 40°/s.** Monocular temporo-nasal: 0.7 / 0.6 / 0.4. **Monocular naso-temporal: ZERO** — no nystagmus could be elicited at any velocity. **The "~80 % of gaze stabilisation is head movement" figure that stood here is WITHDRAWN (#272).** It is not in Masseck et al. and not about geckos: it traces to Dieringer, Cochran & Precht 1983, which measured *Rana* (frog) and turtle, never any squamate, and its ~0.8 is an optokinetic head GAIN, not a head-versus-eye split. Masseck et al. measured HEAD movement only and justified ignoring the eyes by citing that same frog-and-turtle paper. The head/eye split in any gecko is **UNMEASURED**. | [EM] **V** — *this is the best-matched published sensorimotor number for the target species* | absent | Build an OKR loop driving `neck_yaw` + `head_yaw` from optic flow on `head_cam`; tune until head angular velocity ÷ world angular velocity = 0.9 at 20°/s. **Critical: half-wave rectify the flow per hemifield so backward (naso-temporal) flow gives ZERO head-turn command** — otherwise the CPG's own forward walking drives the reflex into runaway. A camera rigidly fixed to a servo'd head remains the right model here for a reason that survives: Masseck et al. measured the HEAD and obtained these gains from it, so a head-driven loop is what the numbers describe. It is **not** supported by any measured head/eye split, because none exists for any gecko. Only validate at 20/30/40°/s; refuse to extrapolate. | Masseck, Röll & Hoffmann 2008 |
...
8. **Masseck, O. A., Röll, B. & Hoffmann, K.-P. (2008).** *The optokinetic reaction in foveate and afoveate geckos.* **Vision Research 48(6):765–772.** — **FREE author PDF.** The **only paper with quantitative sensorimotor measurements on [EM] itself**: hOKR gains at 20/30/40°/s (binocular 0.9 / 0.85 / 0.75; monocular temporo-nasal 0.7 / 0.6 / 0.4) and complete naso-temporal monocular asymmetry — **zero** response at any velocity. It reports NO head/eye split; the "80 % head" attributed to it here was wrong on the paper, wrong on the species and wrong on the quantity (#272). Everything in §3's gaze-stabilisation row comes from here.
